...
- [ ] **New, 15 Aug 2026**: `find_activities()`'s `filtered_db[0]` fallback (`prepare_inventories.py:376-378`) picks an arbitrary, call-unstable electricity-market region whenever a turbine's location matches no available regional dataset — confirmed on Faroe Islands (FO), where two turbines in the same wind farm with identical declared parameters got `{AO}` (Angola) vs `{ID}` (Indonesia) as their Assembly-phase electricity market, driving FO's GWP100 abs-max error to 6.08% (vs ≤0.30% for BY/CY/IS/XK checked the same day, see PLAN's "Completed 15 Aug 2026" section). Decision: document as a known limitation, not fixed — deadline-driven, isolated to FO in the current sample. Related to, but distinct from, the already-documented DE offshore NL-border electricity-market outlier below (that one swaps to a neighboring, at-least-plausible country; this one can land anywhere in ecoinvent's activity list).
